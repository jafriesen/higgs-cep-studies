...
def get_interaction_info(model, bx, interaction):
    idx = np.where((model["universe"][:, 0] == bx) & (model["universe"][:, 1] == interaction))[0]
    if idx.size == 0:
        return None
    idx = idx[0]
    return {
...
def get_primary_vertex(model, bx):
    ...
    if interactions_in_bx.size == 0:
        return None
    idx = np.argmax(model["sum_pt2"][(model["universe"][:, 0] == bx) & np.isin(model["universe"][:, 1], interactions_in_bx)])
    n_tracks = model["n_tracks"][idx]
    if n_tracks == 0:
        return None
    return {
        "interaction_id": model["universe"][idx, 1],
        "z": model["z"][idx],
        "t": model["t"][idx],
        "sum_pt2": model["sum_pt2"][idx],
        "n_tracks": model["n_tracks"][idx],
...
def analyze_bx(bx, model, protons, tracks, args):
    pv = get_primary_vertex(model, bx)
    ...
    bx_protons = get_bx_protons(model, protons, bx)
    ...
    if args.pv_t_res_ps is not None:
        tcut = args.nsigma * math.sqrt(pps_sigma_t_vertex**2 + args.pv_t_res_ps**2)
    else:
        tcut = args.nsigma * args.beam_sigma_z_cm / C_CM_PER_PS
    ...
    bx_p_xi = np.asarray(bx_protons["xi"], dtype=np.float64)
    bx_p_side = np.asarray(bx_protons["side"], dtype=np.int32)
    bx_p_tag200, bx_p_tag400 = station_tags(bx_p_xi)
    ...
    bx_pp = np.array([ (iL, iR) for iL in bx_pL for iR in bx_pR if bx_p_tag400[iL] or bx_p_tag400[iR] ], dtype=np.int32)
    bx_pp_mass = np.array([ np.sqrt(bx_p_xi[iL] * bx_p_xi[iR] * args.s) for iL, iR in bx_pp ], dtype=np.float64)
    ...
    if args.mass_window is not None and args.mass_window >= 0.0:
        ...
        keep = np.ones(len(bx_pp), dtype=bool)
    ...
    interaction_info = {interaction: get_interaction_info(model, bx, interaction) for interaction in np.unique(bx_p_interactions)}
    bx_p_vertices = np.array([ (interaction_info[interaction]["z"], interaction_info[interaction]["t"]) for interaction in bx_p_interactions ], dtype=np.float64)

    bx_pp_vertex_reco = np.zeros((len(bx_pp), 2), dtype=np.float64)
    for pair_idx, (iL, iR) in enumerate(bx_pp):
        pL_vertex = bx_p_vertices[iL]
        pR_vertex = bx_p_vertices[iR]
        pL_t_truth = pL_vertex[1] + pL_vertex[0] / C_CM_PER_PS
        pR_t_truth = pR_vertex[1] - pR_vertex[0] / C_CM_PER_PS
        pL_t_reco = pL_t_truth + rng.normal(loc=0.0, scale=args.pps_time_res_ps)
        pR_t_reco = pR_t_truth + rng.normal(loc=0.0, scale=args.pps_time_res_ps)
        vertex_z_reco = 0.5 * (pL_t_reco - pR_t_reco) * C_CM_PER_PS
        vertex_t_reco = 0.5 * (pL_t_reco + pR_t_reco)
        bx_pp_vertex_reco[pair_idx] = (vertex_z_reco, vertex_t_reco)
    ...
    bx_pp_is_pv = np.array([ (int(bx_p_interactions[iL] == pv["interaction_id"]) + int(bx_p_interactions[iR] == pv["interaction_id"])) for iL, iR in bx_pp ], dtype=np.int32) if pv is not None else np.zeros(len(bx_pp), dtype=np.int32)
    counters = {
        "n_protons": n_protons,
        "n_pp": len(bx_pp),
...
def main():
    ...
    parser.add_argument("--trk-pt-min", type=float, default=0.5, help="Track (charged particles) proxy pT cut in GeV (default: 0.5)")
    parser.add_argument("--trk-eta-max", type=float, default=2.5, help="Track (charged particles) proxy |eta| cut (default: 2.5)")
    parser.add_argument("--beam-sigma-z-cm", type=float, default=5.7, help="Gaussian beam spot sigma z in cm")
    parser.add_argument("--pps-time-res-ps", type=float, default=5.0, help="Single-arm timing resolution in ps")
    parser.add_argument("--pv-z-res-cm", type=float, default=0.2, help="Primary vertex z resolution in cm (central detector)")
    ...
    n_bx = len(np.unique(model["universe"][:, 0]))
    ...
    for bx in np.unique(model["universe"][:, 0]):
        bx_result = analyze_bx(bx, model, protons, tracks, args)
        for key in counters.keys():
            if bx_result["counters"][key] is not None:
                counters[key] += bx_result["counters"][key]
