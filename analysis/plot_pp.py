...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot weighted proton-pair observables for bb or cc tagged Delphes dijet selections."
    )
...
def load_dijet_selection(ak, np, uproot, input_file, tree_name, collection):
    ...
    leading_idx = ak.argmax(selected_pt, axis=1)
    local_idx = ak.local_index(selected_pt, axis=1)
    subleading_pt = ak.where(local_idx == leading_idx, -np.inf, selected_pt)
    subleading_idx = ak.argmax(subleading_pt, axis=1)
    dijet = jets[ak.singletons(leading_idx)][:, 0] + jets[ak.singletons(subleading_idx)][:, 0]
...
def load_protons(np, pythia_file, selected_event_indices, pps_config):
...
def build_observables(np, dijets, protons, pps_config, rng):
    valid = protons["valid"]
    pps = valid & passes_pps(np, protons["xi_left"], pps_config["xi_ranges"])
    pps &= passes_pps(np, protons["xi_right"], pps_config["xi_ranges"])

    xi_left_pps = protons["xi_left"][pps]
    xi_right_pps = protons["xi_right"][pps]
    xi_res = pps_config["xi_res"]
    if xi_res > 0.0:
        xi_left_smeared = rng.normal(xi_left_pps, xi_res)
        xi_right_smeared = rng.normal(xi_right_pps, xi_res)
    else:
        xi_left_smeared = xi_left_pps.copy()
        xi_right_smeared = xi_right_pps.copy()

    smeared_valid = (xi_left_smeared > 0.0) & (xi_right_smeared > 0.0)
    mx_smeared = np.sqrt(xi_left_smeared[smeared_valid] * xi_right_smeared[smeared_valid]) * pps_config["sqrt_s"]
    yx_smeared = 0.5 * np.log(xi_right_smeared[smeared_valid] / xi_left_smeared[smeared_valid])

    return {
        "observables": {
            "mx_before_pps": protons["mx"][valid],
            "yx_before_pps": protons["yx"][valid],
            "mx_after_pps": protons["mx"][pps],
            "yx_after_pps": protons["yx"][pps],
            "mx_smeared_after_pps": mx_smeared,
            "yx_smeared_after_pps": yx_smeared,
            "delta_mx_smeared_minus_dijet": mx_smeared - dijets["dijet_mass"][pps][smeared_valid],
            "delta_yx_smeared_minus_dijet": yx_smeared - dijets["dijet_rapidity"][pps][smeared_valid],
        },
        "n_valid_proton_pairs": int(np.sum(valid)),
        "n_pps": int(np.sum(pps)),
        "n_smeared_valid": int(np.sum(smeared_valid)),
...
def read_samples(ak, np, uproot, processes, parameters, pps_config, args):
    ...
    for process_name in selected_processes(processes, args.include_light_qcd):
        ...
        try:
            ...
            results = build_observables(np, dijets, protons, pps_config, rng)
        ...
        samples.append(
            {
                ...
                "n_pps": results["n_pps"],
                "n_smeared_valid": results["n_smeared_valid"],
        ...
        print(
            ...
            f"pps={results['n_pps']}, smeared_valid={results['n_smeared_valid']}, "
...
def plot_histograms(np, plt, histograms, output_dir, log_y, stacked, normalized=False):
...
def write_plots(np, plt, samples, output_dir, bins, log_y, stacked):
    ...
    for variable, options in PLOT_VARIABLES.items():
        ...
        if plot_histograms(np, plt, histograms, output_dir, log_y, stacked):
            written += 1
        if plot_histograms(np, plt, histograms, output_dir, log_y, stacked, normalized=True):
            written += 1
    return written
...
def main():
    ...
    written = write_plots(np, plt, samples, output_dir, args.bins, args.log_y, args.stacked)
