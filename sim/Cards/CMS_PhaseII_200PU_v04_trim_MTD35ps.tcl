# Trimmed card + MTD35ps timing. Trimmed from CMS_PhaseII_200PU_v04.tcl: only the collections the H(cc)/H(bb)
# MVA reads are produced. Removed the AK8 chains, all BTagging/TauTagging/
# JetFlavorAssociation, the photon and CHS-lepton duplicates, the unused MET
# variants, and the CHS jet chain (its 'Jet' branch was never written).
# Measured on 1200 events: 331.8 -> 221.7 ms/event (-33%), 39.1 -> 24.7 MB (-37%).
# JetPUPPI, EFlowTrack, EFlowTrackPUPPI, GenJet, Particle, Vertex, Electron,
# MuonLoose and PuppiMissingET are unchanged; verified against the full card.
########################################
#
...
# add InputArray InputArray OutputArray
  ...
  # MTD timing: a pileup track is also resolved from the primary vertex when
  # their production times differ by more than TimeSigmaCut sigma. Same values
  # as CMS_PhaseII_200PU_v04_MTD35ps.tcl.
...
# Keep the Vertex collection used by the MVA, but omit its large constituent
# reference arrays. Vertex kinematics, including Vertex.Z, are unchanged.
  set StoreVertexConstituents false
...
# add Branch InputArray BranchName BranchClass
  ...
  add Branch RunPUPPIBase/puppiTracks EFlowTrackPUPPI Track
...
#  add Branch PhotonEfficiencyCHS/photons PhotonCHS Photon
  ...
  # Raw and CHS jets are still reconstructed for dependent modules, but are
  # not serialized; downstream production analysis uses JetPUPPI.
